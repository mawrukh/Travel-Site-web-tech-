...
<script>
  ...
  function saveState() {
    ...
    // Save the state of radio buttons
    var radioButtons = draggableArea.querySelectorAll('input[type="radio"]');
    var checkedRadioButtons = Array.from(radioButtons).filter(button => button.checked);
    var checkedRadioButtonsState = {};
    checkedRadioButtons.forEach(button => {
      checkedRadioButtonsState[button.getAttribute('name')] = button.value;
    });

    localStorage.setItem("draggableAreaContent", JSON.stringify({htmlContent, checkedRadioButtonsState}));
  }

  // Function to load the state of the draggable area
  function loadState() {
    var draggableArea = document.getElementById("draggableArea");
    var storedData = localStorage.getItem("draggableAreaContent");
    if (storedData) {
      storedData = JSON.parse(storedData);
      draggableArea.innerHTML = storedData.htmlContent;

      // Restore the state of radio buttons
      var radioButtons = draggableArea.querySelectorAll('input[type="radio"]');
      Object.entries(storedData.checkedRadioButtonsState).forEach(([name, value]) => {
        var radioButton = Array.from(radioButtons).find(button => button.getAttribute('name') === name && button.value === value);
        if (radioButton) {
  ...
  // Function to send form data to Laravel backend using Fetch API
  function sendDataToBackend(formData) {
    // Make a Fetch API request to your Laravel backend
    fetch('/saveFormData', {
        method: 'POST',
        headers: {
...
<?php /**PATH C:\Users\PMYLS\LaravelProjects\trialTask\task1\resources\views/task.blade.php ENDPATH**/ ?>